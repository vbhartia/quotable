...
letter_page_urls.each do |letter_page|
	...
	#check if need to go to next URL
	#puts check_pagination(all_authors_page)

	all_authors_page.css('table.table.table-hover a').map { |link| puts uRLs_to_index << link['href'] }
	...
	uRLs_to_index.each do |author_page_url|
		...
		File.write('scrapped_quotes.txt', write_to_file, File.size('scrapped_quotes.txt'), mode: 'a')
